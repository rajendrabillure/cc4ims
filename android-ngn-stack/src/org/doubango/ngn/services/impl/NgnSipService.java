		...
		public int OnDialogEvent(DialogEvent e){
			...
			switch (eventCode){
				...
				case tinyWRAPConstants.tsip_event_code_dialog_connected:
				{
                    ...
                    //ALLAGH STON KWDIKA TOU DOUBANGO
                    //TODO: EDW RITHMIZW TO GROUP
                    /*
                    NgnSubscriptionSession subPres = new NgnSubscriptionSession(mSipService.getSipStack(),"sip:group1@organization2.org",NgnSubscriptionSession.EventPackageType.Presence);
                    Log.v(TAG, "GERYON trying to get presence information xmls");
                    subPres.subscribe();
                    
                   
                    NgnSubscriptionSession subPres2 = new NgnSubscriptionSession(mSipService.getSipStack(),"sip:group1@organization2.org",NgnSubscriptionSession.EventPackageType.Presence);
                    Log.v(TAG, "GERYON2 trying to get presence information xmls");
                    subPres2.subscribe();
                    
                    NgnSubscriptionSession subPres3 = new NgnSubscriptionSession(mSipService.getSipStack(),"sip:group3@organization2.org",NgnSubscriptionSession.EventPackageType.Presence);
                    Log.v(TAG, "GERYON3 trying to get presence information xmls");
                    subPres3.subscribe();
                    */
		...
		public int OnSubscriptionEvent(SubscriptionEvent e) {
			...
			switch(type)
			{
				case tsip_i_notify:
				{
					...
					//ALLAGH STON KWDIKA TOU DOUBANGO
					//TODO: EDW EXEI DOULEIA
					Log.v(TAG,"AUTO EINAI SWSTO, PRWTATHLIMA KAI KUPELO EDW");
					 final short code = e.getCode();
                     final String phrase = e.getPhrase();
                     final SipMessage message = e.getSipMessage();
                     Log.v(TAG,phrase);
                     Log.v(TAG,"SIP from: "+message.getSipHeaderValue("From"));
                     Log.v(TAG,"SIP subscription state: "+message.getSipHeaderValue("Subscription-State"));
                     Log.v(TAG,"SIP content type: "+message.getSipHeaderValue("Content-Type"));
                     Log.v(TAG,"SIP content length: "+message.getSipHeaderValue("Content-Length"));
                     Log.v(TAG,"MHKOS: "+message.getSipContentLength()+" POIO: "+message.toString());
                     ...
                     if (content!=null && content.length>0){
                    	 ...
                    	 /*
                     for (int ids=0;ids<content.length;ids++){
                     ...
                     */
                    	...
                    	/* 
                     for (int ids=0;ids<content.length/2;ids++){
                    	 //helper= helper+(char)content[ids];
                    	 helper=helper+(char)content[ids];
                     }    
                     for (int ids=content.length/2;ids<content.length;ids++){
                    	 
                    	 helper2=helper2+(char)content[ids];
                     }    
                     
                     Log.v(TAG,helper);
                     Log.v(TAG,"MHKOS TOU HELPORA: "+helper.length());
                     Log.v(TAG,helper2);
                     Log.v(TAG,"MHKOS TOU HELPORA: "+helper2.length());
                     */
                     ...
                     //ALLAGH STO DOUBANGO KWDIKA*****************************************************************************
                     if (_session == null){
                         NgnSubscriptionSession ngnSession = new NgnSubscriptionSession(mSipService.getSipStack(),message.getSipHeaderValue("From"),NgnSubscriptionSession.EventPackageType.Presence);

                         NgnSubscriptionEventArgs eargs = new NgnSubscriptionEventArgs(ngnSession.getId(), 
                     ...
                     mSipService.broadcastSubscriptionEvent(eargs);
                                          ...
                                          Log.v(TAG,"TO STEILES MWRH MALAKW?");
