...
public class NgnAVSession extends NgnInviteSession {
	...
	public static NgnAVSession takeIncomingSession(NgnSipStack sipStack,
		...
		synchronized (sSessions) {
			NgnAVSession avSession = new NgnAVSession(sipStack, session, media,
	...
	*/
	...
	//CHANGE TO DOUBANGO CODE HERE
	 public static NgnAVSession createOutgoingSession(NgnSipStack sipStack, CallSession session, NgnMediaType mediaType){
	        synchronized (sSessions){
	            final NgnAVSession avSession = new NgnAVSession(sipStack, session, mediaType, InviteState.INPROGRESS);
	            sSessions.put(avSession.getId(), avSession);
	            return avSession;
	        }
	    }
	    ...
	    {
	        return NgnAVSession.createOutgoingSession(sipStack, null, mediaType);
	    }
	...
	private void updateEchoTail() {
		if (mConfigurationService.getBoolean(
			...
			if ((mediaMgr = super.getMediaSessionMgr()) != null) {
				...
				Log.d(TAG, "getMinBufferSize(" + samplingRate + "," + channels
						+ ")=" + minBufferSize);
	...
	public void setModeInCall(boolean bInCall) {
		...
		if (NgnApplication.isSetModeAllowed()) {
			audiomanager.setMode(bInCall ? AudioManager.MODE_IN_CALL
	...
	public void setLocalHold(boolean localHold) {
		...
		if (mVideoProducer != null) {
			...
			Log.v(TAG,"LOCAALHOLD VSTATUS->"+(mLocalHold||mRemoteHold));
		}
		if (mAudioProducer != null) {
			mAudioProducer.setOnPause(mLocalHold || mRemoteHold);
			Log.v(TAG,"LOCAALHOLD ASTATUS->"+(mLocalHold||mRemoteHold));
		}
	...
	// ADDITIONAL CODE, ADDED TO DOUBANGO FRAMEWORK
	protected long mSessionTransferId = -1;

	public static NgnAVSession takeOutgoingTranferSession(NgnSipStack sipStack,
			CallSession session, twrap_media_type_t mediaType,
			SipMessage sipMessage) {
		NgnMediaType media = NgnMediaType.ConvertFromNative(mediaType);
		if (media == NgnMediaType.None) {
		...
		NgnAVSession avSession = NgnAVSession.createOutgoingSession(sipStack,
				session, media);
		if (sipMessage != null) {
			avSession.setRemotePartyUri(sipMessage
	...
	public long getSessionTransferId()
    {
     ...
     {
         if (NgnUriUtils.makeValidSipUri(transferUri) == null)
         {
             return false;
         }
         Log.v(TAG,"TRANSFERRRRROOOOOOOOCALLLLLLLLLLPEONTIKO"+transferUri+"ID="+mSession.getSessionTransferId());
